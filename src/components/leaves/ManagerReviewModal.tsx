      ...
      <div className="relative bg-card w-full max-w-md rounded-xl border border-border shadow-elevated">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="rounded-lg bg-muted/40 border border-border px-3 py-2 text-xs text-muted-foreground">
            <div><span className="font-medium text-foreground">Duration:</span> {leave.startDate} → {leave.endDate} ({leave.days} day{leave.days === 1 ? "" : "s"})</div>
            <div className="mt-1"><span className="font-medium text-foreground">Reason:</span> {leave.reason}</div>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              placeholder={isReject ? "Explain why this request is being rejected." : "Add a note for the employee (optional)."}
