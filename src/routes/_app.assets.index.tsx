...
function EmployeeDashboard() {
  ...
  const mine = useMemo(
    () => assets.filter((a) => a.employee === CURRENT_EMPLOYEE),
    [],
  );
  const assigned = mine.filter(
    (a) => a.status === "assigned" || a.status === "return_requested",
  );
...
function SupportDashboard() {
  ...
  const today = "Jul 11, 2026";

  const pending = assets.filter((a) => a.status === "pending");
  const assignedToday = assets.filter(
    (a) => a.assignedOn === today && (a.status === "assigned" || a.status === "return_requested" || a.status === "returned"),
  );
  const returnedToday = assets.filter((a) => a.returnedOn === today);
  const availableAssets = 18; // Placeholder catalog figure.
    ...
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard
          ...
          value={String(assignedToday.length)}
          delta="Handed out today"
          trend="up"
          icon={<PackageCheck className="h-5 w-5" />}
        />
        <StatCard
          ...
          delta="Ready in inventory"
          icon={<Package className="h-5 w-5" />}
        />
        <StatCard
          label="Returned Today"
          value={String(returnedToday.length)}
          delta="Verified and returned"
